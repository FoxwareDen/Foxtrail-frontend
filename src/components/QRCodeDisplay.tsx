...
const QRCodeDisplay: React.FC<QRCodeDisplayProps> = ({ 
  ...
  const handleClose = () => {
    if (sessionToken) QRAuthService.unsubscribeFromSessionUsage(sessionToken);
    onClose?.();
  };
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeRemaining(prev => (prev <= 1 ? 300 : prev - 1));
    }, 1000);
  ...
  return (
    <div className="flex flex-col items-center space-y-4 p-4 relative text-sm">
      {onClose && (
        <button onClick={handleClose} className="absolute top-2 right-2 p-1 text-gray-400 hover:text-white">
          ✕
        </button>
      )}
      ...
      {loading ? (
        ...
        <div className="w-[200px] h-[200px] bg-red-900/20 border border-red-500 rounded-lg flex flex-col items-center justify-center p-3">
          ...
          <button onClick={generateQRCode} className="px-4 py-1.5 bg-red-500 text-white rounded text-xs">
            Retry
          </button>
        </div>
      ) : (
      ...
      {!mobileScanned && !error && (
        <div className="flex flex-col items-center gap-2">
          ...
          <button 
            ...
            disabled={loading} 
            className="px-4 py-1.5 bg-[#92dce5] text-[#2b303a] rounded text-xs"
          >
            Refresh
          </button>
        </div>
